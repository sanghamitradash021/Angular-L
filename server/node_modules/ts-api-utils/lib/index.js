  ...
  switch (option) {
    ...
    case 'allowSyntheticDefaultImports':
      return options.allowSyntheticDefaultImports !== undefined
        ...
        : isCompilerOptionEnabled(options, 'esModuleInterop') || options.module === ts9.ModuleKind.System;
    case 'alwaysStrict':
    case 'noImplicitAny':
    case 'noImplicitThis':
...
function getAccessKind(node) {
  ...
  switch (parent.kind) {
    ...
    case ts9.SyntaxKind.PrefixUnaryExpression:
      return parent.operator === ts9.SyntaxKind.PlusPlusToken || parent.operator === ts9.SyntaxKind.MinusMinusToken
        ? 3 /* ReadWrite */
        : 1 /* Read */;
...
function isPropertyReadonlyInType(type, name, typeChecker) {
  ...
  for (const subType of unionConstituents(type)) {
    if (getPropertyOfType(subType, name) === undefined) {
      const index =
        (isNumericPropertyName(name) ? typeChecker.getIndexInfoOfType(subType, ts9.IndexKind.Number) : undefined) ??
...
function typeIsLiteral(type) {
  if (isTsVersionAtLeast(5, 0)) {
    ...
    return isTypeFlagSet(type, ts9.TypeFlags.StringLiteral | ts9.TypeFlags.NumberLiteral | ts9.TypeFlags.BigIntLiteral);
  }
}
...
function isReadonlyAssignmentDeclaration(node, typeChecker) {
  ...
  const writableType =
    writableProp.valueDeclaration !== undefined && ts9.isPropertyAssignment(writableProp.valueDeclaration)
