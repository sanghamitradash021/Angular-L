...
function isAssignmentKind(kind) {
  return (
    kind >= ts9__default.default.SyntaxKind.FirstAssignment && kind <= ts9__default.default.SyntaxKind.LastAssignment
  );
}
...
function isInDestructuringAssignment(node) {
  ...
  while (true) {
    switch (node.parent.kind) {
      ...
      case ts9__default.default.SyntaxKind.BinaryExpression:
        return (
          node.parent.left === node && node.parent.operatorToken.kind === ts9__default.default.SyntaxKind.EqualsToken
        );
      case ts9__default.default.SyntaxKind.ForOfStatement:
...
function getWellKnownSymbolPropertyOfType(type, wellKnownSymbolName, typeChecker) {
  ...
  for (const prop of type.getProperties()) {
    ...
    if (prop.escapedName === getPropertyNameOfWellKnownSymbol(typeChecker, globalSymbol, wellKnownSymbolName)) {
      return prop;
    }
  }
  return undefined;
    ...
    if (getPropertyOfType(subType, name) === undefined) {
      const index =
        (isNumericPropertyName(name)
          ...
          : undefined) ?? typeChecker.getIndexInfoOfType(subType, ts9__default.default.IndexKind.String);
      if (index?.isReadonly) {
        if (seenProperty) {
          return true;
        }
...
function symbolHasReadonlyDeclaration(symbol, typeChecker) {
  return !!(
    (symbol.flags & ts9__default.default.SymbolFlags.Accessor) === ts9__default.default.SymbolFlags.GetAccessor ||
    symbol.declarations?.some(
      (node) =>
        ...
        (ts9__default.default.isCallExpression(node) && isReadonlyAssignmentDeclaration(node, typeChecker)) ||
        ts9__default.default.isEnumMember(node) ||
        ((ts9__default.default.isPropertyAssignment(node) ||
          ts9__default.default.isShorthandPropertyAssignment(node)) &&
  ...
  createOrReuseNamespaceScope(name, exported, ambient, hasExportStatement) {
    ...
    this.#innerScope.end((variable, key, scope) => {
      if (
        ...
        (!variable.exported && (!this.#ambient || (this.#exports !== undefined && !this.#exports.has(key.text))))
      ) {
...
var UsageWalker = class {
  ...
  getUsage(sourceFile) {
    ...
    const cb = (node) => {
      ...
      switch (node.kind) {
        ...
        case ts9__default.default.SyntaxKind.EnumMember:
          this.#scope.addVariable(getPropertyName(node.name), node.name, 1 /* Function */, true, 4 /* Value */);
          break;
        case ts9__default.default.SyntaxKind.ExportAssignment:
  ...
  #handleModule(node, next) {
    ...
    if (node.name.kind === ts9__default.default.SyntaxKind.Identifier) {
      ...
      this.#scope.addVariable(node.name.text, node.name, 1 /* Function */, exported, 1 /* Namespace */ | 4 /* Value */);
      const ambient = includesModifier(node.modifiers, ts9__default.default.SyntaxKind.DeclareKeyword);
      return next(
        node,
    ...
    return next(
      node,
      this.#scope.createOrReuseNamespaceScope(`"${node.name.text}"`, false, true, namespaceHasExportStatement(node)),
    );
  }
  #handleVariableDeclaration(declarationList) {
    ...
    const exported =
      ...
      includesModifier(declarationList.parent.modifiers, ts9__default.default.SyntaxKind.ExportKeyword);
